...
class ShuffleNet(object):
    ...
    def _build(self):
        ...
        conv1 = self._conv2d('conv1', x=x_padded, w=None, num_filters=self.output_channels['conv1'], kernel_size=(3, 3),
        ...
        max_pool = self._max_pool_2d(padded, size=(3, 3), stride=(2, 2), name='max_pool')
        stage2 = self._stage(max_pool, stage=2, repeat=3)
        stage3 = self._stage(stage2, stage=3, repeat=7)
        stage4 = self._stage(stage3, stage=4, repeat=3)
        global_pool = self._avg_pool_2d(stage4, size=(7, 7), stride=(1, 1), name='global_pool', padding='VALID')

        logits_unflattened = self._conv2d('fc', global_pool, w=None, num_filters=self.args.num_classes,
                                    kernel_size=(1, 1),
                                    l2_strength=self.args.l2_strength,
                                    bias=self.args.bias,
                                    is_training=self.is_training)
        self.logits = self._flatten(logits_unflattened)
    ...
    def _stage(self, x, stage=2, repeat=3):
        if 2 <= stage <= 4:
            stage_layer = self._shufflenet_unit('stage' + str(stage) + '_0', x=x, w=None,
            ...
            for i in range(1, repeat + 1):
                stage_layer = self._shufflenet_unit('stage' + str(stage) + '_' + str(i),
    ...
    def _grouped_conv2d(self, name, x, w=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
        ...
        with tf.variable_scope(name) as scope:
            ...
            conv_side_layers = [
                self._conv2d(name + "_" + str(i), x[:, :, :, i * sz:i * sz + sz], w, num_filters // num_groups, kernel_size,
                       padding,
                       stride,
                       initializer,
    ...
    def _depthwise_conv2d_p(self, name, x, w=None, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
    ...
    def _depthwise_conv2d(self, name, x, w=None, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
        ...
        with tf.variable_scope(name) as scope:
            conv_o_b = self._depthwise_conv2d_p(name='conv', x=x, w=w, kernel_size=kernel_size, padding=padding,
    ...
    def _shufflenet_unit(self, name, x, w=None, num_groups=1, group_conv_bottleneck=True, num_filters=16, stride=(1, 1),
        ...
        with tf.variable_scope(name) as scope:
            ...
            if group_conv_bottleneck:
                bottleneck = self._grouped_conv2d('Gbottleneck', x=x, w=None, num_filters=bottleneck_filters, kernel_size=(1, 1),
                                            padding='VALID',
                                            num_groups=num_groups, l2_strength=l2_strength, bias=bias,
                                            activation=activation,
                                            batchnorm_enabled=batchnorm_enabled, is_training=is_training)
                shuffled = self._channel_shuffle('channel_shuffle', bottleneck, num_groups)
            else:
                bottleneck = self._conv2d('bottleneck', x=x, w=None, num_filters=bottleneck_filters, kernel_size=(1, 1),
                                    padding='VALID', l2_strength=l2_strength, bias=bias, activation=activation,
                                    batchnorm_enabled=batchnorm_enabled, is_training=is_training)
                shuffled = bottleneck
            padded = tf.pad(shuffled, [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT")
            depthwise = self._depthwise_conv2d('depthwise', x=padded, w=None, stride=stride, l2_strength=l2_strength,
                                         padding='VALID', bias=bias,
                                         activation=None, batchnorm_enabled=batchnorm_enabled, is_training=is_training)
            if stride == (2, 2):
                residual_pooled = self._avg_pool_2d(residual, size=(3, 3), stride=stride, padding='SAME')
            else:
                residual_pooled = residual

            if fusion == 'concat':
                group_conv1x1 = self._grouped_conv2d('Gconv1x1', x=depthwise, w=None,
                ...
                if num_filters != residual_pooled.get_shape()[3].value:
                    residual_match = self._conv2d('residual_match', x=residual_pooled, w=None, num_filters=num_filters,
                                            kernel_size=(1, 1),
    ...
    def _channel_shuffle(self, name, x, num_groups):
        with tf.variable_scope(name) as scope:
    ...
    def _conv2d_p(self, name, x, w=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
                  initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0, bias=0.0):
        with tf.variable_scope(name):
            stride = [1, stride[0], stride[1], 1]
    ...
    def _conv2d(self, name, x, w=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
               initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0, bias=0.0,
               activation=None, batchnorm_enabled=False, max_pool_enabled=False, dropout_keep_prob=-1,
               is_training=True):
        with tf.variable_scope(name) as scope:
            conv_o_b = self._conv2d_p('conv', x=x, w=w, num_filters=num_filters, kernel_size=kernel_size,
            ...
            conv_o = conv_o_dr
            if max_pool_enabled:
                conv_o = self._max_pool_2d(conv_o_dr)
        return conv_o

    def _variable_with_weight_decay(self, kernel_shape, initializer, wd):
        """
    ...
    # Pooling Methods
    def _max_pool_2d(self, x, size=(2, 2), stride=(2, 2), name='pooling'):
        """
    ...
    def _avg_pool_2d(self, x, size=(2, 2), stride=(2, 2), name='avg_pooling', padding='VALID'):
        """
    ...
    def _dense_p(self, name, x, w=None, output_dim=128, initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0,
                  bias=0.0):
        """
        ...
        with tf.variable_scope(name):
            if w == None:
                w = self._variable_with_weight_decay([n_in, output_dim], initializer, l2_strength)
            self._variable_summaries(w)
            if isinstance(bias, float):
    ...
    def _dense(self, name, x, w=None, output_dim=128, initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0,
              bias=0.0,
              activation=None, batchnorm_enabled=False, dropout_keep_prob=-1,
              is_training=True
              ):
        ...
        """
        with tf.variable_scope(name) as scope:
            dense_o_b = self._dense_p(name='dense', x=x, w=w, output_dim=output_dim, initializer=initializer,
